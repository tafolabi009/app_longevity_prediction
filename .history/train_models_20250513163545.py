...
def main():
    ...
    parser.add_argument("--skip-lightgbm", action="store_true", 
                       help="Skip training LightGBM model (use if encountering feature name issues)")
    ...
    try:
        ...
        # Handle LightGBM skipping if requested
        if args.skip_lightgbm and 'lgb' in model.models:
            print("Skipping LightGBM training as requested")
            del model.models['lgb']
        
        # Apply monkey patch to train_models method to handle LightGBM errors
        original_train_models = model.train_models
        
        def patched_train_models(std_data, lstm_data=None):
            try:
                return original_train_models(std_data, lstm_data)
            except Exception as e:
                if "lightgbm" in str(e).lower() and "feature name" in str(e).lower():
                    print(f"LightGBM error detected: {str(e)}")
                    print("This is likely due to special characters in feature names.")
                    ...
                    # Re-run training but skip LightGBM
                    return original_train_models(std_data, lstm_data)
                else:
                    # Re-raise if it's not a LightGBM feature name error
                    raise
        
        # Apply the monkey patch
        model.train_models = patched_train_models
