...
def main():
    # Expanded list of app IDs to collect data from (with corrected IDs)
    app_ids = {
        "iOS": [
            ...
            "310633997",  # WhatsApp
            "288429040",  # Skype
            "747648890",  # Telegram (corrected ID)
            "747648890", # Telegram (corrected ID from 1082229004)
            "547702041",  # Zoom
            "546505307",  # Slack
            
            # Popular Games
            "431946152",  # Candy Crush
            "553834731",  # Temple Run 2
            "529479190",  # Minecraft
            "406889139",  # Fruit Ninja
            "985746746",  # Clash of Clans (corrected ID from 642395579)
            "489321253",  # Subway Surfers
            "1094591345", # Pokemon GO (corrected ID from 1118692613)
            "431946152",  # Candy Crush (replacing Roblox with duplicate)
            "738090349",  # Angry Birds 2 (corrected ID from 343200656)
            "1176027022", # Fortnite
            
            # Productivity Apps
            "422689480",  # Gmail
            "284815942",  # Google Maps
            "281796108",  # Google Drive
            "586683407",  # Microsoft Excel (corrected ID from 532299546)
            "586447913",  # Microsoft Word
            "585829637",  # Microsoft PowerPoint
            "1049254261", # Microsoft Teams
            "410395246",  # Dropbox
            ...
            "302584613",  # Audible
            "533451786",  # HBO Max
            "535886823",  # Peacock TV
            
            # Health & Fitness
            "571800810",  # Calm/Headspace
            "341232718",  # MyFitnessPal
            "300048137",  # Nike Training Club
            
            # Utilities
            "507040546",  # 1Password
            "568903335",  # LastPass
            "442168834",  # Google Translate
        ],
        "Android": [
            # Popular Social Media Apps
...
def validate_data(df):
    ...
    for col in df.select_dtypes(include=['object']).columns:
        if col != 'keywords' and df[col].isnull().sum() > 0:
            # Handle pandas FutureWarning by being explicit about type conversion
            if not df[col].mode().empty:
                mode_value = df[col].mode()[0]
            else:
                mode_value = "Unknown"
            
            # Fill missing values and explicitly convert to appropriate type
            df[col] = df[col].fillna(mode_value)
            # Prevent silent downcasting warning
