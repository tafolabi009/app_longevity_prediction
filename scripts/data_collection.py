...
def validate_app_ids(app_ids, platform):
    """Validates app IDs to filter out invalid ones before processing."""
    valid_ids = []
    
    if platform == "iOS":
        for app_id in app_ids:
            try:
                # Check if app exists with iTunes API
                ...
                if data and 'results' in data and len(data['results']) > 0:
                    ...
                    print(f"Skipping invalid iOS app ID: {app_id}")
            except Exception as e:
                print(f"Error validating iOS app ID {app_id}: {e}")
    else:  # Android
        for app_id in app_ids:
            try:
                # Use Google Play Scraper to check if app exists
                import time
                try:
                    from google_play_scraper import app as play_app
                    app_details = play_app(app_id, lang='en', country='us')
                    if app_details and 'title' in app_details:
                        valid_ids.append(app_id)
                    else:
                        print(f"Skipping invalid Android app ID: {app_id}")
                except Exception as e:
                    if "not found" in str(e).lower() or "404" in str(e):
                        print(f"Skipping invalid Android app ID: {app_id}")
                    else:
                        # If it's a network error, we still consider it potentially valid
                        valid_ids.append(app_id)
                        print(f"Warning validating Android app ID {app_id}: {e}")
                
                # Add delay to avoid rate limiting
                time.sleep(1)
            except Exception as e:
                print(f"Error validating Android app ID {app_id}: {e}")
...
def main():
    ...
    print(f"Validating {len(app_store_ids)} iOS app IDs...")
    valid_ios_ids = validate_app_ids(app_store_ids, "iOS")
    print(f"Found {len(valid_ios_ids)} valid iOS app IDs out of {len(app_store_ids)}")
    
    print(f"Validating {len(play_store_ids)} Android app IDs...")
    valid_android_ids = validate_app_ids(play_store_ids, "Android")
    print(f"Found {len(valid_android_ids)} valid Android app IDs out of {len(play_store_ids)}")
    ...
    with ThreadPoolExecutor(max_workers=3) as executor:  # Reduced workers to avoid rate limits
        futures = []
        for platform, ids in {"iOS": valid_ios_ids, "Android": valid_android_ids}.items():
            for app_id in ids:
